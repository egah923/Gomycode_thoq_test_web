...
import { Button, Flex, Form, Grid, Input, Select, Spin, Typography, Upload } from "antd";
...
const EditContestent = (props) => {
  ...
  // ************************ Set Contestent Form ************************
  useEffect(() => {
    const { contestent } = props;
    const contestentData = contestent?.filter((res) => res?._id === router?.query?.contestent_id)?.[0];
    ...
    if (contestentData) {
      ...
      // Set Cover Media 
      if (contestentData.material) {
        form.setFieldsValue({
          material: {
            fileList: [{
              uid: contestentData.material,
              name: contestentData.material,
              status: 'done',
              url: henceforthApi.FILES.imageMedium(contestentData.material),
            }]
          },
        });
      }
  
      if (contestentData.profile_pic) {
        form.setFieldsValue({
          profile_pic: {
            fileList: [{
              uid: contestentData.profile_pic,
              name: contestentData.profile_pic,
              status: 'done',
              url: henceforthApi.FILES.imageMedium(contestentData.profile_pic),
            }]
          },
        });
      }
  
      if (contestentData.video_intro) {
        const videoFileUrl = henceforthApi.FILES.video(contestentData.video_intro);
        form.setFieldsValue({
          video_intro: {
            fileList: [{
              uid: contestentData.video_intro,
              name: contestentData.video_intro,
              status: 'done',
              url: videoFileUrl,
            }]
          },
      ...
      if (contestentData.contest_act) {
        form.setFieldsValue({
          contest_act: {
            fileList: [{
              uid: contestentData.contest_act,
              name: contestentData.contest_act,
              status: 'done',
              url: henceforthApi.FILES.video(contestentData.contest_act),
            }]
          },
        });
      }
    }
  }, [props, router.query.contestent_id, form]);
  ...
  const onFinish = async (values: any) => {
    ...
    let payLoad = {
      contestent_id:router?.query?.contestent_id,
    ...
    try {
      setFormLoading(true);
      const apiRes = await henceforthApi.Contestent.editContestentByCreator(payLoad);
      Toast.success('Form submitted successfully.')
      console.log(apiRes);

      router.replace({
        pathname: `/profile/contest-created`,
        // query: {
...
EditContestent.getLayout = (page: ReactNode) => (
  <RootLayout>
    {page}
  </RootLayout>
);

export default EditContestent