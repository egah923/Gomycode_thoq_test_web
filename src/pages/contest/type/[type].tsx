...
const Contests = () => {
    ...
    console.log(contestsData?.draftListing, "draftListing");
    ...
    const submitContestents = (_id, filling_type) => {
        router.replace(
            {
                pathname: '/contest/type/participants-form',
                query: {
    ...
    const items: TabsProps['items'] = [
        ...
        {
            key: 'draft',
            label: <Flex wrap='nowrap' align='center' gap={4}><Typography.Text>Drafts</Typography.Text> <TabBadge>{contestsData?.draftListing?.data.length}</TabBadge></Flex>,
            children: <ProfileWrapperElement>
                <Spin spinning={loading} tip size="large">
                    {contestsData?.draftListing?.data?.length > 0 ?
                        <Row gutter={[24, 40]}>
                            {
                                Array.isArray(contestsData?.draftListing?.data) && contestsData?.draftListing?.data?.map((res: ContestCardProps) => {
            ...
            <Col span={24}>
                <WrapperElement>
                    <div className='d-flex justify-content-around'>
                        <h3 className='fw-bold text-black'>Contest</h3>
...
Contests.getLayout = (page: ReactNode) => (
    <RootLayout noFooter>
        {page}
    </RootLayout>
);
